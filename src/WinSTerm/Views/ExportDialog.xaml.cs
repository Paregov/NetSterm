...
public partial class ExportDialog : MetroWindow
{
    ...
    private void BuildConnectionsTree()
    {
        ...
        foreach (var folder in store.Folders)
        {
            ...
            if (folder.ParentFolderId != null && folderMap.TryGetValue(folder.ParentFolderId, out var parent))
            {
                item.Parent = parent;
    ...
    private static void CollectSelectedIds(ExportTreeItem item, string itemType, HashSet<string> ids)
    {
        if (item.ItemType == itemType && item.IsChecked == true && !string.IsNullOrEmpty(item.Id))
        {
            ids.Add(item.Id);
        }
...
public class BoolToFolderIconConverter : IValueConverter
{
    ...
    {
        return value is true ? PackIconMaterialKind.FolderOutline : PackIconMaterialKind.FileDocumentOutline;
    }
